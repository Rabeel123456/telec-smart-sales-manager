...
export default function Documents({ type }) {
  ...
  const [rows,setRows]=useState([]), [quotations,setQuotations]=useState([]), [deliveries,setDeliveries]=useState([])
  ...
  async function load(){
    ...
    if(error)return alert(error.message);setRows(data||[]);setQuotations(qs||[]);setDeliveries(ds||[])
  }
  ...
  const nextNumber=()=>`${settings?.[cfg.prefixKey]||({quotation:'QT',delivery:'DC',invoice:'INV'})[type]}-${new Date().getFullYear()}-${String(rows.length+1).padStart(4,'0')}`
  const initialForm=()=>({
  ...
  function fromQuotation(id){
    ...
    setForm(f=>({...f,quotation_id:q.id,opportunity_id:q.opportunity_id,user_id:q.user_id,customer_name:q.customer_name,contact_person:q.contact_person||'',customer_address:q.customer_address||'',items:q.items||[blankItem()],vendor:q.vendor||'',vendor_terms:q.vendor_terms||'',probability:q.probability||75,remarks:''}))
  }
  function fromDelivery(id){
    const d=deliveries.find(x=>x.id===id);if(!d)return
    setForm(f=>({...f,delivery_challan_id:d.id,quotation_id:d.quotation_id,opportunity_id:d.opportunity_id,user_id:d.user_id,customer_name:d.customer_name,contact_person:d.contact_person||'',customer_address:d.customer_address||'',items:d.items||[blankItem()],vendor:d.vendor||'',vendor_terms:d.vendor_terms||'',probability:100,remarks:''}))
  }
  ...
  async function save(e){
    ...
    try{
      ...
      const payload={
        user_id:profile.role==='admin'?form.user_id:profile.id, opportunity_id:opportunityId, document_no:form.document_no.trim(), document_date:form.document_date,
